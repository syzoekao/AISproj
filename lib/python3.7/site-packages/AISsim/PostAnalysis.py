...
for k in keys[:-1]: 
	...
	plt.tight_layout()
	plt.savefig("results/"+k+'(new).eps', format='eps', dpi=1000)
plt.close()
...
ax4.text(np.mean(temp0), 0.15, str(np.round(np.mean(temp0)*1000,2))+"x$10^{-3}$", fontsize=15)
...
lake_risk_zm.to_csv('results/risk table (zm)(new2).csv')
...
lake_risk_ss.to_csv('results/risk table (ss)(new2).csv')
...
with open('results/effect summary stats (zm)(new2).txt', 'w') as fout:
    json.dump(summary_zm, fout)

with open('results/scenario effect (zm)(new2).txt', 'w') as fout:
...
with open('results/effect summary stats (ss)(new2).txt', 'w') as fout:
    json.dump(summary_ss, fout)

with open('results/scenario effect (ss)(new2).txt', 'w') as fout:
...
plt.tight_layout()
plt.savefig("results/boat_infestation(new2).eps", format='eps', dpi=1000)
...
plt.tight_layout()
plt.savefig("results/river_infestation(new2).eps", format='eps', dpi=1000)
...
for s in ['E', 'P', 'D', 'T']: 
	...
	plt.tight_layout()
	plt.savefig("results/boat_infestation("+s+")(new2).eps", format='eps', dpi=1000)
...
for s in ['E', 'P', 'D', 'T']: 
	...
	plt.tight_layout()
	plt.savefig("results/river_infestation("+s+")(new2).eps", format='eps', dpi=1000)
...
plt.tight_layout()
plt.savefig('results/management strategies(new2).eps', format='eps', dpi=1000)
