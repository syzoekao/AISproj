...
def infest_outcome_func(factor_ss, e_violate_zm, e_violate_ss, river_inf_zm, river_inf_ss, \
	back_suit_zm, back_suit_ss, boat_net, # small_prob, 
	river_o, river_d, river_w, lake_id, infest_zm, \
	infest_ss, infest_both, zm_suit, ss_suit): 
	# bbb = timeit.default_timer()

	'''
	factor_ss=0.00280901
	e_violate_zm=0.00070804
	e_violate_ss=0.03522039 
	river_inf_zm=0.00578524
	river_inf_ss=0.00647215
	back_suit_zm=0.01
	back_suit_ss=0.01
	...
	zm_count = {2010: 36,  2011: 40,  2012: 53,  2013: 68,  2014: 84,  2015: 99,  2016: 132,  2017: 167, 2018: 202}
	...
	ix = ix.astype(np.intp)
	if ix.shape[0] > 0:
		yy = np.array((2018-2016)*[0])
		temp = temp.loc['id'].values
		yy[ix] = temp
		ann_ss = np.cumsum(yy) + ss_count[2016]
	else: 
		ann_ss = np.array((2018-2016)*[ss_count[2016]])
...
def pre_infest_outcome_func(factor_ss, e_violate_zm, e_violate_ss, river_inf_zm, river_inf_ss, back_suit_zm, back_suit_ss, \
	boat_net, small_prob, river_o, river_d, river_w, lake_id, infest_zm, 
	infest_ss, infest_both, zm_suit, ss_suit): 
	bbb = timeit.default_timer()

	'''
	factor_ss=0.09678538
	e_violate_zm=0.00095045
	e_violate_ss=0.0018413 
	river_inf_zm=0.49019133
	river_inf_ss=0.17441344
	back_suit_zm=0.19838811
	back_suit_ss=0.25602098
	'''
	...
	week = 26
	year = 2018-2012 # starting year is 2013 and the initial condition is based on the end of 2012 (zebra mussels)
	time_horizon = week*year
	...
	boat_key_o = np.array([k for k,v in boat_net.items()])
	small_key_o = np.array([k for k,v in small_prob.items()])
	...
	for t in range(1, time_horizon+1): 
		...
		for x in zm_lake: 
			ix_d = []
			if x in boat_key_o: 
				k0, w0 = [np.array([k for k, v in boat_net[x].items()]), np.array([v for k, v in boat_net[x].items()])]
			else: 
				k0, w0 = [np.array([]), np.array([])]
			if x in small_key_o: 
				k1, w1 = [np.array([k for k, v in small_prob[x].items()]), np.array([v for k, v in small_prob[x].items()])]
			else: 
				k1, w1 = [np.array([]), np.array([])]
			tmp_k = np.where(np.in1d(k1, k0))[0]
			k1 = np.delete(k1, tmp_k)
			w1 = np.delete(w1, tmp_k)
			w1 = np.random.binomial(1, w1)
			k1 = k1[w1>0]
			w1 = w1[w1>0]
			k = np.concatenate([k0, k1]).astype(np.int)
			k = k.astype(np.intp)
			w = np.concatenate([w0, w1]).astype(np.int)
			w = np.random.binomial(w, o_violate_zm*e_violate_zm)*tmp_suit_zm0[k]
...
def Scenario(factor_ss, e_violate_zm, e_violate_ss, river_inf_zm, river_inf_ss, back_suit_zm, back_suit_ss, \
	boat_net, small_prob, river_o, river_d, river_w, lake_id, infest_zm, \
	infest_ss, infest_both, tmp_suit_zm, tmp_suit_ss, \
	scenario = "StatusQuo", pReduceTraffic = 0.5, result2dict = True): 
	bbb = timeit.default_timer()
	...
	week = 26
	year = 2025-2018 # starting year is 2013 and the initial condition is based on the end of 2012 (zebra mussels)
	time_horizon = week*year
	...
	# for t in range(1, 10): 
		...
		if scenario == "ReduceTraffic": 
			for x in zm_lake: 
				...
				k1 = k1[w1>0]
				w1 = w1[w1>0]
				k = np.concatenate([k0, k1]).astype(np.intp)
				w = np.concatenate([w0, w1])
				tot_boat = np.sum(w)*(1-pReduceTraffic0)
				prop = w/np.sum(w)
	...
	if ix.shape[0] > 0: 
		...
		temp = temp.loc['id'].values
		yy[ix] = temp
		ann_zm = np.cumsum(yy) + zm_count[2018]
	else: 
		ann_zm = np.array((year)*[zm_count[2018]])
	...
	if ix.shape[0] > 0: 
		...
		temp = temp.loc['id'].values
		yy[ix] = temp
		ann_ss = np.cumsum(yy) + ss_count[2018]
	else: 
		ann_ss = np.array((year)*[ss_count[2018]])
